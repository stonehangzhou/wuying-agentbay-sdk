    ...
    try:
        ...
        try:
            ...
            if 'success' in judgement.lower().split('status:')[1]:  # This is the official criteria for success
                evaluation = {'task_id': task_folder.name, 'judgement': judgement,
                              'success': True, 'error': None, 'score': 1.0}
            else:  # This is the official criteria for failure
                evaluation = {'task_id': task_folder.name, 'judgement': judgement,
                              'success': False, 'error': None, 'score': 0.0}
...
async def run_agent_with_browser(
    ...
    memory_config = None
    if enable_memory:
        memory_config = MemoryConfig(agent_id=f'eval_agent_{task.task_id}',
                                     memory_interval=memory_interval, llm_instance=llm)
        ...
        try:
            ...
            try:
                    ...
                    if agent_history is not None:  # Only format if agent ran successfully
                        try:
                            ...
                            formatted_data = await run_stage(
                                ...
                                lambda: reformat_agent_history(agent_history, task.task_id,
                                                               run_id, task.confirmed_task),
                            )
                ...
                try:
                    ...
                    await run_stage(
                        Stage.SAVE_SERVER,
                        lambda: asyncio.to_thread(save_result_to_server, convex_url,
                                                  secret_key, task_result.server_payload),
                        timeout=60,
                    )
            ...
            _logger.critical(
                f'Task {task.task_id}: Catastrophic initialization error: {str(init_error)}', exc_info=True)
